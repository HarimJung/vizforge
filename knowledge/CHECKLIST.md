...
- [ ] 데이터 단위 일관성 검증: Sankey/flow 차트는 모든 레이어가 동일 단위인지 확인
- [ ] 세부 데이터가 공개 데이터셋인지 추정치인지 구분. 추정치면 출처+방법 명시
...
## 3.5 스토리텔링 & 컨텍스트 (Stage 5에서 점검)

- [ ] 차트 아래에 "Key Insights" 섹션 존재 (최소 2~3개 인사이트 카드)
- [ ] 각 인사이트에 비교 맥락 포함 (예: "X = Y국 전체 소비량")
- [ ] 핵심 발견 문장이 데이터에서 도출된 구체적 수치를 포함
- [ ] 마지막에 "The Bottom Line" 또는 요약 메시지 1문단 존재
- [ ] 출처 표기에 보고서명 + 연도 포함 (단체명만 불가)
...
- [ ] 필터/노드 클릭 시 KPI 숫자가 해당 부분합으로 카운트업 전환
- [ ] 필터 버튼 활성 상태: 카테고리 컬러 배경 + white 텍스트 + glow shadow
- [ ] "전체" 클릭 시 원래 KPI 값으로 복원 애니메이션
- [ ] 필터/노드 클릭 시 노드 라벨 숫자도 부분합으로 전환 (KPI뿐 아니라 차트 내 라벨)
- [ ] "전체" 클릭 시 모든 노드 라벨 값 원래대로 복원
- [ ] 파티클 흐름은 circle + getPointAtLength 방식 사용 (stroke-dashoffset 파티클 아님)
...
- [ ] 숫자 포맷 통일: 큰 수 콤마 표기, 단위 명시 (Mt CO₂, B liters, TWh)
- [ ] 레이어/섹션 헤더와 데이터 라벨 간 타이포 위계 분리 확인
- [ ] 차트 여백에 레이어별 소계 또는 카테고리별 서브토탈 표시 여부 검토
- [ ] SVG 내 text-transform은 JS로 처리 (CSS 미지원)
