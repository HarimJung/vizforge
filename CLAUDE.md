...
4. 시각적 결정(색상, 타이밍, 여백)은 knowledge/COMBOS-INDEX.md 및 해당 COMBO 스펙의 레퍼런스 수치를 따른다.
...
- 디자인 무드를 knowledge/DESIGN.md에서 매칭한다.
- 결과를 사용자에게 제시한다: "이 데이터에는 [조합] + [Nadieh 기법들] + [디자인 무드]를 추천합니다."
...
- 포함 항목: 프로젝트 컨셉(한 문단), 데이터 스키마, CSS 디자인 시스템(변수 + 폰트 + 위계), HTML 레이아웃(와이어프레임), 인터랙션 흐름(단계별), 애니메이션 시퀀스(타이밍 + ease), 적용 이펙트 명세.
- 모든 시각적 수치는 매칭된 COMBO 스펙의 레퍼런스에서 가져온다.
- ⏸️ 사용자 확인: "이 설계 문서를 리뷰해주세요. 수정할 부분이 있나요?"

### Stage 4 — Code Generation (3단계 분할)
- 4-A: HTML + CSS 뼈대. JS 없음. 레이아웃과 디자인 시스템만.
